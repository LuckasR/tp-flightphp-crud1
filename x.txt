...
<div class="container">
  ...
  <table id="table-remboursements" class="table table-bordered table-hover text-center align-middle">
    ...
    <tbody>
      <tr>
        <td colspan="9" class="text-muted">Sélectionnez un prêt et générez le plan de remboursement.</td>
      </tr>
    </tbody>
  </table>
</div>
...
function genererPlanRemboursement() {
  ...
  ajax("GET", `/prets/${id_pret}`, null, pret => {
    ...
    const tauxAssurance = Number(pret.frais_assurance || 0);
    const montantAccorde = Number(pret.montant_accorde || 0);
    const dureeInitiale = Number(pret.duree_accordee);
    ...
    ajax("GET", `/paiements`, null, paiements => {
      ...
      const dureeRestante = Math.max(0, dureeInitiale - moisPayes);
      if (dureeRestante === 0) {
        alert("Ce prêt est entièrement remboursé.");
        document.querySelector("#table-remboursements tbody").innerHTML = `<tr><td colspan="9">Aucun remboursement restant.</td></tr>`;
        return;
      }

      const annuite = calculerAnnuite(montant_restant, tauxAnnuel, dureeRestante);
      const assuranceMensuelle = (tauxAssurance / 100) * montantAccorde / dureeInitiale;
      ...
      for (let i = 1; i <= dureeRestante; i++) {
        ...
        const paiementTotal = annuite + assuranceMensuelle;
        ...
        tr.innerHTML = `
          ...
          <td>${assuranceMensuelle.toFixed(2).replace('.', ',')}</td>
          <td>${paiementTotal.toFixed(2).replace('.', ',')}</td>
          <td><button onclick="enregistrerPaiement(${id_pret}, ${id_admin}, ${paiementTotal.toFixed(2)}, '${dateEcheanceStr}')">Enregistrer</button></td>
        `;
...
function enregistrerPaiement(id_pret, id_admin, montant_paye, date_valeur) {
  const data = `id_pret=${id_pret}&id_admin=${id_admin}&montant_paye=${montant_paye}&date_valeur=${date_valeur}&commentaire=Annuité constante`;
